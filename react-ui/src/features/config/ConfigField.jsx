import { checkboxes, colors, labels, selectOptionLabels, selectOptions } from "./configSchema.js";
...
export default function ConfigField({ section, field, value, onChange }) {
    ...
    const input = selectOptions[field]
        ? <select className={controlClass} value={value ?? ""} onChange={(event) => onChange(section, field, event.target.value)}>{selectOptions[field].map((option) => <option key={option} value={option}>{selectOptionLabels[field]?.[option] || option}</option>)}</select>
