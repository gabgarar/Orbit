...
function PassesTable({ passes }) {
    ...
    return <div className="mt-2 overflow-x-auto rounded-[7px] border border-[#265143] bg-[rgba(5,20,17,.42)]">
...
function PassElevationChart({ result }) {
    ...
    return <section className="mt-3 rounded-[9px] border border-[#203b59] bg-[linear-gradient(155deg,rgba(8,19,35,.96),rgba(5,13,25,.96))] p-2.5">
        ...
        <svg ref={svgRef} className="block min-h-[250px] w-full select-none" viewBox={`0 0 ${width} ${height}`} role="img" aria-label="Elevación del satélite durante el pase seleccionado">
            ...
            <g clipPath="url(#pass-elevation-clip)"><path d={path} fill="none" stroke="url(#pass-elevation-line)" strokeWidth="2.25" strokeLinecap="round" strokeLinejoin="round" />{samples.map((sample, index) => {
                const previous = samples[index - 1];
                return index && sample.visible && previous?.visible ? <line key={`${previous.time}-${sample.time}`} x1={x(previous.time)} y1={y(previous.elevation_deg)} x2={x(sample.time)} y2={y(sample.elevation_deg)} stroke="#6ff0a1" strokeWidth="3" vectorEffect="non-scaling-stroke" /> : null;
            })}</g>
