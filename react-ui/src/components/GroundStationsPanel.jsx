...
function PassElevationChart({ result }) {
    ...
    const width = 340; const height = 132; const pad = { left: 27, right: 9, top: 12, bottom: 23 };
    ...
    return <div className="mt-3 rounded-md border border-[#285345] bg-[#061611] px-1.5 py-1.5">
        <div className="mb-1 flex items-center justify-between px-1 text-[9px] font-bold tracking-wide text-[#a9d8bf]"><span>PERFIL DE ELEVACIÓN</span><span>UTC · 30 s</span></div>
        <svg className="block h-auto w-full" viewBox={`0 0 ${width} ${height}`} role="img" aria-label="Elevación del satélite sobre la estación durante las próximas 24 horas">
            ...
            <text x={pad.left} y={height - 6} fill="#8aab9c" fontSize="8">inicio</text><text x={width - pad.right} y={height - 6} textAnchor="end" fill="#8aab9c" fontSize="8">+24 h</text>
        </svg>
...
export default function GroundStationsPanel() {
    ...
    useEffect(() => {
        ...
        const openPasses = (event) => { setStationId(String(event.detail?.stationId || "")); setOpen(true); refresh(); };
    ...
    return <aside className="fixed top-[82px] right-3 z-[10140] flex max-h-[calc(100dvh-102px)] w-[min(390px,calc(100vw-24px))] flex-col overflow-hidden rounded-[12px] border border-[#2b4e7b] bg-[rgba(7,16,31,.96)] font-[system-ui,sans-serif] text-[#e7f0ff] shadow-[0_18px_48px_rgba(0,0,0,.48)]" aria-label="Operaciones de estaciones terrestres">
        <header className="flex items-center justify-between border-b border-[#203956] px-3 py-2.5"><div><h2 className="m-0 text-[13px] font-bold tracking-wide">GROUND STATIONS</h2><p className="mt-0.5 mb-0 text-[10px] text-[#8ea4c4]">Operación de pases y visibilidad</p></div><button type="button" onClick={() => setOpen(false)} className="grid size-7 place-items-center rounded-md border border-[#294565] bg-transparent text-[#b9c9df] hover:bg-[#13233a]" aria-label="Cerrar">×</button></header>
        <div className="grid gap-3 overflow-y-auto p-3">
            <section className="rounded-lg border border-[#203956] bg-[rgba(13,29,51,.7)] p-2.5"><div className="mb-2 flex items-center justify-between"><strong className="text-[11px]">Estación activa</strong><button type="button" onClick={() => { setOpen(false); window.dispatchEvent(new Event("orbit:ground-stations-create-request")); }} className="rounded-md border border-[#466cff] bg-[#263f96] px-2 py-1 text-[10px] font-bold text-white">+ Añadir</button></div>{state.stations.length ? <select className={inputClass} value={stationId} onChange={(event) => { setStationId(event.target.value); setResult(null); }}>{state.stations.map((item) => <option value={item.id} key={item.id}>{item.name} · {item.latitude_deg.toFixed(3)}°, {item.longitude_deg.toFixed(3)}°</option>)}</select> : <p className="m-0 text-[11px] text-[#aabbd2]">Crea una estación para iniciar un análisis.</p>}{station && <><p className="mb-0 text-[10px] text-[#8ea4c4]">Máscara actual: {station.min_elevation_deg}° · ITRF/WGS‑84</p><button type="button" onClick={() => { setMonitoringStationId(station.id); setMonitoredIds(station.monitor_satellite_ids || []); }} className="mt-2 rounded border border-[#315d92] bg-transparent px-2 py-1 text-[10px] font-bold text-[#a8c7ff]">Satélites monitorizados ({station.monitor_satellite_ids?.length || 0})</button></>}</section>
