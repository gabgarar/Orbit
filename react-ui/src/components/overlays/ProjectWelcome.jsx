...
function RuntimeFailureNotice() {
    return <div className="mt-5 grid gap-2 rounded-xl border border-[#ff7171ad] bg-[#71141e75] p-[14px] text-left font-sans text-[13px] leading-[1.45] text-[#ffd8d8]" role="alert">
...
export default function ProjectWelcome({ onAction, runtimeStatus, startup }) {
    ...
    >
        ...
        <div className={`relative z-10 max-h-[calc(100dvh-40px)] ${preparing ? "w-[min(620px,calc(100vw-40px))]" : "w-[min(510px,calc(100vw-40px))]"} overflow-y-auto rounded-[30px] border border-[#71a9e694] bg-[linear-gradient(135deg,rgba(9,40,77,.91),rgba(3,13,27,.88))] px-[42px] pt-[54px] pb-11 text-center shadow-[0_32px_90px_rgba(0,0,0,.58)] [scrollbar-color:#426589_transparent] [scrollbar-width:thin] max-[520px]:px-6 max-[520px]:pt-9`}>
            ...
            <h1 className="!mt-[30px] !mb-[18px] !font-sans !text-[clamp(34px,3vw,48px)] !leading-[1.1] !font-semibold !text-white">{preparing ? "Preparando Orbit" : "Welcome to Orbit"}</h1>
            ...
            {preparing ? <section className="text-left" data-testid="startup-preparing-view" aria-label="Preparando Orbit">
                <p className="!m-0 !mb-5 text-center font-sans text-base leading-[1.55] font-normal text-[#b9c9df]">Orbit está descargando y validando los datos necesarios antes de permitir crear o abrir un proyecto.</p>
                {runtimeFailed && <RuntimeFailureNotice />}
                <StartupStatusPanel startup={startup} />
                {!runtimeFailed && <p className="!mt-4 !mb-0 text-center font-sans text-xs leading-[1.45] text-[#9eb6d4]">Este estado se actualiza automáticamente. La escena y el Built-In Test siguen disponibles.</p>}
            </section> : <>
