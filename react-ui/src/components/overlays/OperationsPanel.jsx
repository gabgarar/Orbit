...
function OperationItem({ operation }) {
    ...
    return <article className="rounded-[9px] border border-[#294767] bg-[#09172a] px-3 py-2.5 text-[#dce8fb]">
        <div className="flex min-w-0 items-start gap-2">
            <span className="mt-1.5 size-2 shrink-0 animate-pulse rounded-full bg-[#70a1ff] shadow-[0_0_8px_rgba(91,142,255,.92)]" aria-hidden="true" />
            <div className="min-w-0 flex-1">
                <div className="flex min-w-0 items-start gap-2">
                    <strong className="min-w-0 flex-1 text-[12px] leading-[1.25] font-semibold text-[#edf5ff]">{operation.title}</strong>
                    <span className="shrink-0 rounded-full border border-[#375c89] bg-[#102744] px-1.5 py-0.5 text-[9px] leading-none font-semibold text-[#a9c8ff]">{scopeLabel(operation.scope)}</span>
                </div>
                <div className="mt-1 flex items-center gap-1.5 text-[10px] leading-none text-[#8fa9cf]"><span className="rounded-full border border-[#315171] bg-[#0e2239] px-1.5 py-0.5 font-semibold text-[#b4cef7]">{statusLabel(operation.status)}</span>{operation.stage ? <span className="min-w-0 truncate">{operation.stage}</span> : null}</div>
                {operation.message ? <p className="mt-1 mb-0 text-[10px] leading-snug text-[#8fa7c9]">{operation.message}</p> : null}
                <div className="mt-2 flex items-center gap-2">
                    <div className="h-1.5 min-w-0 flex-1 overflow-hidden rounded-full bg-[#172842]" aria-label={`Progreso: ${progressText(operation.progress)}`}>
                        <div className={`h-full rounded-full bg-[#5b8eff] transition-[width] duration-200 ${hasProgress ? "" : "w-2/5 animate-pulse"}`} style={hasProgress ? { width: `${operation.progress}%` } : undefined} />
                    </div>
...
export default function OperationsPanel({ operations, onClose }) {
    ...
    return <section
        ...
        className="fixed top-[calc(max(64px,calc(76px*var(--orbit-ui-scale)))+10px)] right-[clamp(10px,1.7vw,28px)] z-[10520] max-h-[min(520px,calc(100vh-max(64px,calc(76px*var(--orbit-ui-scale)))-26px))] w-[min(440px,calc(100vw-20px))] overflow-auto rounded-[12px] border border-[#38577f] bg-[#091628]/[.98] p-3 text-[#dce8fb] shadow-[0_22px_60px_rgba(0,0,0,.58)] backdrop-blur-[8px]"
        role="dialog"
        aria-modal="false"
        aria-labelledby="orbitOperationsTitle"
    >
        <header className="mb-3 flex items-center justify-between gap-3 border-b border-[#27415f] pb-2.5">
        ...
        </div> : <p className="m-0 rounded-[8px] border border-[#294667] bg-[#0d2038] px-3 py-3 text-[11px] leading-snug text-[#afc1dc]">No hay operaciones activas.</p>}
    </section>;
}
