...
function startupDiagnosticsFinished(snapshot) {
    ...
    // A transient NGA/download error is terminal for that individual check,
    // but not for the service: it may publish a later retry and recovery.
    // Poll until the backend explicitly permits project actions.
    return Boolean(component && getStartupProjectReadiness(startupStatusFromDiagnosticComponent(component)).ready);
}
...
export default function App() {
    ...
    // This short-lived poll is only for the startup ledger. It stops only
    // after the service explicitly enables project actions, so transient
    // download failures and their automatic retries remain visible.
